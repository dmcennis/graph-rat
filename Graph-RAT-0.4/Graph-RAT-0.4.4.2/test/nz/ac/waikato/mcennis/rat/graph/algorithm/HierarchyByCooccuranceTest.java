...
public class HierarchyByCooccuranceTest extends TestCase {
    ...
    protected void setUp() throws Exception {
        ...
        props.setProperty("userActor", "artist");
        props.setProperty("artistActor", "tag");
        prepare.init(props);
    ...
    public void testExecuteRock() {
        ...
        Link[] general = base.getLinkBySource("Generalization", rock);
        assertNull(general);
        
        Link[] specialization = base.getLinkBySource("Specialization", rock);
        assertNotNull(specialization);
        assertEquals(1,specialization.length);
        assertEquals(alternative,specialization[0].getDestination());
        
        Link[] disjoint = base.getLinkBySource("Disjoint", rock);
        assertNotNull(disjoint);
        assertEquals(4,disjoint.length);
        for(int i=0;i<disjoint.length;++i){
            if(disjoint[i].getDestination().getID().contentEquals("Blues")){
                
            }else if(disjoint[i].getDestination().getID().contentEquals("OldBlues")){
                
            }else if(disjoint[i].getDestination().getID().contentEquals("Bebop")){
                
            }else if(disjoint[i].getDestination().getID().contentEquals("Jazz")){
                
            }else{
                fail("Unexpected tag '"+disjoint[i].getDestination().getID()+"' detected in set of disjoint links");
            }
        }
    }
    ...
    public void testExecuteAlternative() {
        ...
        Link[] general = base.getLinkBySource("Generalization", alternative);
        assertNotNull(general);
        assertEquals(1,general.length);
        assertEquals(rock,general[0].getDestination());
        
        Link[] specialization = base.getLinkBySource("Specialization", alternative);
        assertNull(specialization);
        
        Link[] disjoint = base.getLinkBySource("Disjoint", alternative);
        assertNotNull(disjoint);
        assertEquals(4,disjoint.length);
        for(int i=0;i<disjoint.length;++i){
            if(disjoint[i].getDestination().getID().contentEquals("Blues")){
    ...
    public void testExecuteJazz() {
        ...
        Link[] general = base.getLinkBySource("Generalization", jazz);
        assertNull(general);
        
        Link[] specialization = base.getLinkBySource("Specialization", jazz);
        assertNotNull(specialization);
        assertEquals(3,specialization.length);
         for(int i=0;i<specialization.length;++i){
            if(specialization[i].getDestination().getID().contentEquals("Blues")){
                
            }else if(specialization[i].getDestination().getID().contentEquals("OldBlues")){
                
            }else if(specialization[i].getDestination().getID().contentEquals("Bebop")){
                
            }else{
                fail("Unexpected tag '"+specialization[i].getDestination().getID()+"' detected in set of specialization links");
            }
        }
        
        Link[] disjoint = base.getLinkBySource("Disjoint", jazz);
        assertNotNull(disjoint);
        assertEquals(2,disjoint.length);
        for(int i=0;i<disjoint.length;++i){
            if(disjoint[i].getDestination().getID().contentEquals("Rock")){
                
            }else if(disjoint[i].getDestination().getID().contentEquals("Alternative")){
    ...
    public void testExecuteBlues() {
        ...
        Link[] general = base.getLinkBySource("Generalization", blues);
        assertNotNull(general);
        assertEquals(1,general.length);
        assertEquals(jazz,general[0].getDestination());
        
        Link[] specialization = base.getLinkBySource("Specialization", blues);
        assertNotNull(specialization);
        assertEquals(1,specialization.length);
        assertEquals(oldBlues,specialization[0].getDestination());
        
        Link[] disjoint = base.getLinkBySource("Disjoint", blues);
        assertNotNull(disjoint);
        assertEquals(3,disjoint.length);
        for(int i=0;i<disjoint.length;++i){
            if(disjoint[i].getDestination().getID().contentEquals("Rock")){
    ...
    public void testExecuteOldBlues() {
        ...
        Link[] general = base.getLinkBySource("Generalization", oldBlues);
        assertNotNull(general);
        assertEquals(2,general.length);
        for(int i=0;i<general.length;++i){
            if(general[i].getDestination().getID().contentEquals("Blues")){
                
            }else if(general[i].getDestination().getID().contentEquals("Jazz")){
                
            }else{
                fail("Unexpected tag '"+general[i].getDestination().getID()+"' detected in set of general links");
            }
        }
        
        Link[] specialization = base.getLinkBySource("Specialization", oldBlues);
        assertNull(specialization);
        
        Link[] disjoint = base.getLinkBySource("Disjoint", oldBlues);
        assertNotNull(disjoint);
        assertEquals(3,disjoint.length);
        for(int i=0;i<disjoint.length;++i){
            if(disjoint[i].getDestination().getID().contentEquals("Rock")){
    ...
    public void testExecuteBebop() {
        ...
        Link[] general = base.getLinkBySource("Generalization", bebop);
        assertNotNull(general);
        assertEquals(1,general.length);
        assertEquals(jazz,general[0].getDestination());
        
        Link[] specialization = base.getLinkBySource("Specialization", bebop);
        assertNull(specialization);
        
        Link[] disjoint = base.getLinkBySource("Disjoint", bebop);
        assertNotNull(disjoint);
        assertEquals(4,disjoint.length);
        for(int i=0;i<disjoint.length;++i){
            if(disjoint[i].getDestination().getID().contentEquals("Rock")){
                
            }else if(disjoint[i].getDestination().getID().contentEquals("Blues")){
