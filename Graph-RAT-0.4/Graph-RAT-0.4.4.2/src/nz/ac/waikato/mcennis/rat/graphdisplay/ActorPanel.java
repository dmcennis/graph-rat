...
//        innerPanel.setBackground(ColorLib.getColor(-26215));
        ...
        JLabel modeLabel = new JLabel(actor.getType());
...
//        id.setMinimumSize(new Dimension(300,30));
        ...
        Property[] propertyList = actor.getProperty();
        if (propertyList != null) {
            for (int i = 0; i < propertyList.length; ++i) {
                ...
                if (propertyList[i].getValue()[0].toString().length() > 38) {
                    bigEntries.put(propertyList[i].getType(), new DisplayLargeStringPropertyFrame(propertyList[i]));
                    JButton bigEntry = new JButton(propertyList[i].getValue()[0].toString().substring(0, 35)+"...");
