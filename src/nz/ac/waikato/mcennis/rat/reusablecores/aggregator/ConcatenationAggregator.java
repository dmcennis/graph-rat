...
public class ConcatenationAggregator implements AggregatorFunction{
    ...
    public Instance[] aggregate(Instance[] data, double[] weights) {
        ...
        if((data != null)&&(data.length>0)&&(weights != null)&&(weights.length == data.length)){
            ...
            if(data==null){
                Logger.getLogger(ConcatenationAggregator.class.getName()).log(Level.WARNING, "Instance array is null");
            }else if(weights==null){
                Logger.getLogger(ConcatenationAggregator.class.getName()).log(Level.WARNING, "weights array is null");
            }else if(data.length != weights.length){
                Logger.getLogger(ConcatenationAggregator.class.getName()).log(Level.SEVERE, "Array lengths differ - "+data.length+":"+weights.length);
            }
        }
        return ret;
    }

    public ConcatenationAggregator prototype(){
        return new ConcatenationAggregator();
    }
}
